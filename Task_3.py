...
max = (v1+v2)/2
print("max",round(max,8))
plt.scatter(max,[(R*T)/(max-b)-a/(max**2)], color = "red")
for i in range(0,len(V)): #paint oranege forbidden line
    if V[i]>= min:
        V=V[i:]
        P=P[i:]
        break
for i in range(0, len(V)):
    if V[i] >= max:
        ...
        P = P[:i]
        break
plt.plot(V,P, color = "orange")
length = 0
for i in range(1, len(V)):
    length += ((V[i]-V[i-1])**2+(P[i]-P[i-1])**2)**0.5
print("length:",round(length,6))
print("Qualiti scatters:",len(V))
plt.show()
